...
const PayAmount = () => {
  ...
  const { payAmount, accountNumberList, loading, error } = useSelector(
  ...
  if (accountNumberList?.length < 1) {
    return (
      <ContainerTile loading={loading} error={error} className="text-center">
        No Pay Amount Account
      </ContainerTile>
    );
  }
  ...
  return (
    <>
      ...
      {payAmount?.accountStatus && (
        <ContainerTile className={"mt-5"}>
          <div className="grid grid-cols-2 gap-4 text-[14px] pb-2">
            {Object.entries(payAmount?.accountStatus).map(([key, value]) => (
              <InfoRow
                key={key}
