...
const TestComponent = () => {
  ...
  const handleRangeChange = (e, index) => {
    ...
    setFormData((prevData) => ({
      ...prevData,
      numberCriteriaRangeList: prevData.numberCriteriaRangeList.map(
        (item, idx) => {
          if (index === idx) {
            return {
              ...item,
  ...
  const deleteRangeEntry = (index) => {
    setFormData((prevData) => ({
      ...prevData,
      numberCriteriaRangeList: prevData.numberCriteriaRangeList.filter(
        (_, idx) => idx !== index
      ),
    }));
  };
      ...
      <div className="grid grid-cols-1 gap-4 flex-1">
        <div className={`grid gap-5 px-5 grid-cols-3`}>
          ...
          <InputSelect
            labelName="Name"
            inputOptions={
              formData?.fieldType === "STRING" ? StringArray : NumberArray
            }
            inputName="name"
          ...
          {formData.numberCriteriaRangeList.map((range, index) => (
            ...
            >
              ...
              {formData?.fieldType === "NUMBER" && (
                <div
                  className={
                    "flex justify-center items-center absolute -top-3 -right-2 px-2 bg-red-500 rounded-xl font-bold hover:bg-red-400 hover:cursor-pointer"
                  }
